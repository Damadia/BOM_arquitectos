<?php
	...
	$success = false; // Variable del documento para hacer evaluaciones si se está registrado
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") 
		{
			...
			{
				$query = "INSERT INTO usuario VALUES('".$email."','".$name."','".$cell."','".$password."', ".$tier.", '".date('Y/m/d')."')";
	...
	<div class="bigFormContainer">
		<?php		
			if ($_SERVER["REQUEST_METHOD"] == "POST")
				...
				{
					echo "<h2 class='successLegend'>El registro ha sido exitoso, el usuario ya puede <a href='logInForm.html''>iniciar sesión</a></h2>";
				}

		?>
		<form action="insertUser.php" id="formForRegister" method="post" class="form1Style">
			<p class="TitleForm">Completa los datos el registro</p>
			<div class="fields">	
				...
				<p class="centerField">
					<input class="hyStyle" type="submit" value="Registrar">
				</p>
			</div>	
		</form>
	</div>
